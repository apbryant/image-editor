...
void doGridEffect(PGraphics pg, int nCols, int nRows, int w, int h, int cd, int md) {
  ...
  for (int y = 0; y < nRows; y++) {
    for (int x = 0; x < nCols; x++) {
      if (random(1) < 0.001 && cd < md) {
        ...
        int sx = x * colWidth;
        int sy = y * rowHeight;
        int dx = sx + int(map(noise(sx, x, cd), 0, 1, width * -0.1, width * 0.1));
        int dy = sy + int(map(noise(sy, y, cd), 0, 1, width * -0.1, width * 0.1));
...
void doPaintingEffect(PGraphics pg, int nCols, int nRows, int w, int h, int cd, int md) {
  int colWidth = w / nCols;
  int rowHeight = h / nRows;
